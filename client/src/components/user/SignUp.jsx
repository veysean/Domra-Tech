...
const SignUpCard = () => {
  ...
  const handleSignUp = async (e) => {
    ...
    try{
      ...
      const successMessage = res?.data?.message || "Registered successfully";
      ...
      if (error.response?.status === 500 && !serverMessage) {
        navigate("/", { replace: true });
        return;
      }
      setError(serverMessage || "Sign-up failed. Please try again.");
    } finally {
      setLoading(false);
    }
  };
