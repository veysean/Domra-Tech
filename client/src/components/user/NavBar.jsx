            ...
            <div className="w-full bg-white border-b border-[#E2E8F0] px-6 py-4">
                <div className="relative flex justify-between items-center lg:px-10 mx-auto w-full">
                    ...
                    <div className="lg:flex justify-between  items-center w-full max-w-[1200px]">
                        ...
                        <div className="hidden ml-auto mr-50 lg:flex gap-10">
                            {navItems.map((item) => (
                                <NavLink
                                    ...
                                    to={item.path}
                                    className={({ isActive }) =>
                                        `h-[28px] text-main-color text-[20px] font-inter cursor-pointer ${
                                        isActive ? "text-main-color2 underline underline-offset-4 decoration-[#E4A54D]" : ""
                                        }`
                                    }
                                    >
                                    {item.label}
                        ...
                        {auth ? (
                            <Link to="/profile" className="px-2.5 py-2 main-color rounded-[30px] flex justify-center items-center">
                                <div className="text-white text-xs lg:text-[20px] font-medium font-inter">{t("profile")}</div>
                            </Link>
                            ) : (
                            <Link to="/auth" className="w-auto px-2 lg:px-2.5 py-2 main-color rounded-[30px] flex justify-center items-center">
                        ...
                        <div className="relative">
                            ...
                            {isLangOpen && (
                                ...
                                {["ENG", "ខ្មែរ"].map((lang) => (
                                    ...
                                    className="px-5 py-2.5 text-main-color text-base lg:text-[20px] font-semibold font-inter hover:bg-gray-100 cursor-pointer flex justify-between items-center border-t first:border-t-0 border-[#E2E8F0]"
                                    >
                                    {lang}
                                    {language === lang && (
                    ...
                    <div className="lg:hidden ml-3">
                        <button onClick={toggleMenu} className="text-main-color">
                    ...
                    <div className="absolute top-full right-0 mt-2 w-[180px] bg-[#F8F9FA] shadow-md rounded-2xl overflow-hidden z-50 transition-all duration-300 ease-in-out">
                        ...
                        <NavLink
                            ...
                            className={({ isActive }) =>
                                `px-5 py-2.5 text-main-color text-sm md:text-base lg:text-[18px] font-inter hover:bg-gray-100 cursor-pointer flex justify-between items-center border-t first:border-t-0 border-[#E2E8F0] ${
                                isActive ? "underline underline-offset-4 decoration-main-color font-semibold" : ""
                                }`
                            }
                            >
                            {item.label}
